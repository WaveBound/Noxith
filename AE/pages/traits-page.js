...
export async function TraitsPage(filter = "") {
  ...
  // Hide page content if traits tab is not published yet
  if (!IS_TRAITS_PUBLISHED) {
    page.innerHTML = `
      <div class="traits-locked-container glass-card">
        ...
        <h1 class="traits-locked-title">Traits Catalog Coming Soon</h1>
        <p class="traits-locked-sub">The Trait Database is currently under construction and will be published shortly.</p>
      </div>
    `;
    return page;
  }
  ...
  page.innerHTML = `
    <div class="traits-page-header">
      <div class="traits-header-meta">
        <h1>Traits Database</h1>
        <div class="page-subtitle">View unit traits, drop rates, pity thresholds, and stat modifiers</div>
      </div>
  ...
  async function renderFilteredGrid() {
    ...
    if (filteredTraits.length === 0) {
      container.innerHTML = `<div class="traits-empty-state">No traits match the selected filters.</div>`;
      return;
    }
