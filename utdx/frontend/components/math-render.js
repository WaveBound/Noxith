...
const fmt = {
    ...
    pctMult: (n, bpMult) => {
        if (!n && n !== 0) return '';
        const base = bpMult && bpMult > 1.01 ? n / bpMult : n;
        if (bpMult && bpMult > 1.01) {
            return `<span style="font-size:0.85em; opacity:0.6;">${base > 0 ? '+' : ''}${parseFloat(base.toFixed(1))}% × ${parseFloat(bpMult.toFixed(2))} = </span><span>${n > 0 ? '+' : ''}${parseFloat(n.toFixed(1))}%</span>`;
        }
        return `${(n || 0) >= 0 ? '+' : ''}${parseFloat((n || 0).toFixed(1))}%`;
    }
};
...
function getActiveTagAndSourceForStat(statType, data) {
        ...
        if (isAbh) {
            if (statType === 'dmg' || statType === 'cdmg') {
                if (unitTags.includes('Fused') || unitTags.includes('Fusion')) {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Fusion Tag - ABH)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Fusion Tag - ABH)`, found: true };
                }
            }
            if (statType === 'dmg' || statType === 'spa') {
                if (unitTags.includes('Super Warrior')) {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Super Warrior Tag - ABH)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Super Warrior Tag - ABH)`, found: true };
                }
            }
            if (statType === 'dmg' || statType === 'cf') {
                if (element === 'light') {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Light Element - ABH)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Light Element - ABH)`, found: true };
                }
            }
        } else if (isTt) {
            if (statType === 'dmg' || statType === 'range') {
                if (unitTags.includes('Piece')) {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Piece Tag - Triple Threat)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Piece Tag - Triple Threat)`, found: true };
                }
            }
            if (statType === 'dmg' || statType === 'range') {
                if (unitTags.includes('Sword')) {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Sword Tag - Triple Threat)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Sword Tag - Triple Threat)`, found: true };
                }
            }
            if (statType === 'dmg' || statType === 'cf') {
                if (element === 'wind') {
                    return { label: `↳ Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Wind Element - Triple Threat)`, critLabel: `• Leader Buff<br>&nbsp;&nbsp;&nbsp;&nbsp;(Wind Element - Triple Threat)`, found: true };
                }
            }
        } else if (isKs) {
            if (statType === 'dmg' || statType === 'spa') {
                if (unitTags.includes('Magi')) return { dmg: 50, spa: 15 };
                if (unitTags.includes('Uncontrollable Power')) return { dmg: 30, spa: 10 };
    ...
    if (matchingTags.length > 0) {
        ...
        return { label: `↳ Tag Bonus<br>&nbsp;&nbsp;&nbsp;&nbsp;(${tagStr} : ${sourceStr})`, critLabel: `• Tag Bonus<br>&nbsp;&nbsp;&nbsp;&nbsp;(${tagStr} : ${sourceStr})`, found: true };
    }
        ...
        <div class="math-section" style="border:1px solid rgba(251,191,36,0.2); border-left:4px solid #fbbf24; padding:8px 12px; background:linear-gradient(135deg,rgba(251,191,36,0.05) 0%,rgba(255,255,255,0) 100%);">
            ...
            <div class="math-row" style="margin-bottom:6px; border-bottom:1px solid rgba(255,255,255,0.03); padding-bottom:4px;">
                ...
                <div style="text-align:right;">
                    ...
                    ${data.placement < (data.baseStats?.placement || 1) ? `<div style="font-size:0.5rem; color:#f87171; font-weight:700;">(Synergy Reduction)</div>` : ''}
                </div>
            </div>
            <div class="math-row" style="margin-bottom:6px; border-bottom:1px solid rgba(255,255,255,0.03); padding-bottom:4px;">
...
function renderSourceTotalsSection(data) {
    ...
    let globalPassiveDmg = 0, globalPassiveSpa = 0, globalPassiveRange = 0, globalPassiveCrit = 0, globalPassiveCdmg = 0;
    let globalBuffsDmgHtml = '', globalBuffsSpaHtml = '', globalBuffsCritHtml = '';
    ...
    const combinedActiveTags = [...new Set([...(data.totalSetStats?.activeTags || []), ...(data.headBuffs?.activeTags || [])])];
    ...
    return `
        ...
        (window.isUnit?.(data.baseStats?.id, 'nutaru_beast') ? `
            ...
            if (data.detailedBuffs && data.detailedBuffs.passiveBreakdown && data.detailedBuffs.passiveBreakdown.length > 0) {
                ...
                if (Math.abs(rem) > 0.01) html += `<div style="display:flex; justify-content:space-between; font-size:0.65rem; color:#999;"><span>Unit Base</span><span class="text-white">${fmt.pct(rem)}</span></div>`;
                return html;
            } else {
                const eternalSub = (data.traitObj && data.traitObj.isEternal) ? (Math.min(data.wave || 12, 12) * 5) : 0;
                ...
                return val > 0 ? `<div style="display:flex; justify-content:space-between; font-size:0.65rem; color:#999;"><span>Unit Passive</span><span class="text-white">${fmt.pct(val)}</span></div>` : '';
                    ...
                    ${globalBuffsDmgHtml}${globalBuffsSpaHtml}${globalBuffsCritHtml}
                </div>
                <div style="grid-column:span 1; display:grid; grid-template-columns:1fr 1fr; gap:4px 12px; font-size:0.65rem; color:#777;">
                    ...
                    <div style="display:flex; justify-content:space-between;"><span>Crit Rate</span><b style="color:#60a5fa;">+${globalPassiveCrit}%</b></div>
                    <div style="display:flex; justify-content:space-between;"><span>Crit Dmg</span><b style="color:#60a5fa;">+${globalPassiveCdmg}%</b></div>
                </div>
            </div>
        </div>`;
}
...
function renderActiveBuffsSection(data) {
    ...
    if (data.activeGlobalBuffs && window.GLOBAL_BUFF_DATA) {
        Object.values(window.GLOBAL_BUFF_DATA).forEach(buff => {
            if (data.activeGlobalBuffs[buff.id]) {
                buffs.push({ name: "Global: " + buff.name, desc: buff.renderLabel || buff.desc || '', color: buff.color });
            }
        });
    }

    if (data.traitObj?.isEternal) buffs.push({ name: "Eternal Stacks", desc: "Applied: +5% Dmg & +2.5% Rng / Wave (Max 12)", color: "#c084fc" });
    if (data.traitObj?.hasRadiation) buffs.push({ name: "Radiation", desc: "Fission: Radiation active on enemies", color: "#f87171" });
    if (data.conditionalData) buffs.push({ name: data.conditionalData.name, desc: `Target condition: x${data.conditionalData.mult.toFixed(2)} Dmg`, color: "#fb923c" });
    ...
    if (data.headBuffs && data.headBuffs.note) buffs.push({ name: "Accessory Passive", desc: data.headBuffs.note, color: "#f472b6" });
    ...
    if (activePassives && Array.isArray(activePassives)) {
        activePassives.forEach(p => {
            let stats = [];
            const pb = (data.detailedBuffs && data.detailedBuffs.passiveBreakdown) ? data.detailedBuffs.passiveBreakdown.find(item => item.name === p.name) : null;
            if (pb) {
                ...
                if (pb.dot) {
                    ...
                    stats.push(`+${pb.dot}% DoT${dotType ? ' (' + dotType + ')' : ''}`);
                }
...
function renderQuickBreakdownSection(data, avgHitPerUnit, dotColorClass) {
    ...
    return `
        <div class="math-section no-border-bottom" style="margin-bottom:4px;">
            ...
            ? `Stances Weighted Avg × ${data.placement}`
            : (data.baseStats?.id === 'triple_threat'
                ...
                <div style="border-color:${data.dot > 0 ? 'rgba(192,132,252,0.3)' : (isInactive ? 'rgba(239,68,68,0.4)' : '#333')};">
                    ...
                    <div class="mq-sub">${data.dot > 0 ? (data.hasStackingDoT ? `Stacking: x${data.placement} units` : `Limited: x1 unit`) : (isInactive ? `Needs ${data.dotData?.requirement}` : 'No DoT')}</div>
                </div>
                ...
                ${data.summon > 0 ? `<div style="border-color:rgba(96,165,250,0.3);"><div class="mq-label text-accent-start">${isNutaru ? 'Clone' : (data.summonData?.isCustom ? 'Custom Summon' : 'Plane')} DPS</div><div class="mq-val text-accent-start">${fmt.num(data.summon)}</div><div class="mq-sub">Independent of Host</div></div>` : `<div style="border-color:rgba(96,165,250,0.3);"><div class="mq-label text-accent-start">Attack Rate</div><div class="mq-val text-accent-start">${fmt.fix(data.spa, 2)}s</div><div class="mq-sub">Base: ${data.baseStats?.spa}s (Cap: ${data.spaCap}s)</div></div>`}
            </div>
        </div>`;
}
...
function renderBaseDamageSection(data, levelMult, traitRowsDmg, dmgAfterRelic, preConditionalDmg, baseSetDmg, tagDmg, passiveDmg, eternalDmg, statPointsHtml, cleanHeadDisplayName, relicSetName) {
    ...
    if (data.activeGlobalBuffs && window.GLOBAL_BUFF_DATA) {
        Object.values(window.GLOBAL_BUFF_DATA).forEach(buff => {
            ...
            if (bData && bData.dmg > 0) {
                globalDmgBreakdownHtml += `<tr><td class="mt-cell-label mt-pl-md opacity-70" style="color:${buff.color};">↳ ${buff.name}</td><td class="mt-cell-formula" style="color:${buff.color};">${fmt.pct(bData.dmg / (data.bpMult || 1))}</td><td class="mt-cell-val"></td></tr>`;
            }
        });
    }
    ...
    if (data.warlordData) {
        ...
        <tr class="mt-row-warlord"><td colspan="3" class="p-2">
            ...
            <div class="mt-flex-between mt-border-top mt-pt-sm">
                <span class="text-white text-xs text-bold">Avg Damage Buff (+${fmt.fix(45 * (w.starMult || 1), 2)}% * Uptime)</span>
                <span class="text-sm mt-text-bold" style="color:#a78bfa;"> +${fmt.fix(w.dmg, 1)}%</span>
            </div>
        </td></tr>`;
    }

    if (data.headBuffs && (data.headBuffs.type === 'almighty_accessory' || data.headBuffs.type === 'almighty')) {
        const activeTags = data.headBuffs.activeTags || [];
        const tagRows = activeTags.map(tag => {
            const perkList = (typeof TAG_PERKS !== 'undefined') ? (TAG_PERKS.almighty_acc || []) : [];
            const perk = perkList.find(p => p.tag === tag);
            if (!perk) return '';
            const b = perk.bonus || {};
            let parts = [];
            if (b.dmg) parts.push(`+${b.dmg}% DMG`);
            if (b.dot) parts.push(`+${b.dot}% DoT`);
            if (b.cRate) parts.push(`+${b.cRate}% Crit Rate`);
            return `<div class="mt-flex-between text-xs text-white mb-1"><span class="opacity-70">↳ ${tag}</span><span class="mt-font-mono" style="color:#c4b5fd;">${parts.join(', ')}</span></div>`;
        }).join('');
        headPieceHtml += `
        <tr class="mt-row-ninja" style="background:rgba(167,139,250,0.08); border-left:3px solid #a78bfa;"><td colspan="3" class="p-2">
            <div class="mt-flex-between mb-2"><span class="mt-text-bold text-xs tracking-sm" style="color:#c4b5fd;">ALMIGHTY ACCESSORY</span><button class="calc-info-btn" onclick="openInfoPopup('dot_logic')">?</button></div>
            <div class="mt-flex-between text-xs text-white mb-1"><span class="opacity-70">Critical Hits:</span><span class="mt-font-mono" style="color:#f87171;">Disabled</span></div>
            ${tagRows}
            <div class="mt-flex-between mt-border-top mt-pt-sm"><span class="text-white text-xs text-bold">Global Buff Multiplier</span><span class="mt-text-bold" style="color:#c4b5fd;">×1.30</span></div>
        </td></tr>`;
    }
    ...
    return `
            <div class="dd-section">
                ...
                <table class="calc-table">
                    ...
                    <tr>
                        ...
                        <td class="mt-cell-formula mt-pt-md mt-text-gold mt-text-bold" style="white-space:nowrap;">${fmt.pctMult(data.totalAdditivePct, data.bpMult)}</td>
                        <td class="mt-cell-val calc-highlight mt-pt-md">${fmt.num(preConditionalDmg)}</td>
                    </tr>
                    ${(data.detailedBuffs ? data.detailedBuffs.setBase : baseSetDmg) !== 0 ? `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Set Base Bonus <span style="font-size: 0.65rem; color:#a78bfa;">(${relicSetName})</span></td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(data.detailedBuffs ? data.detailedBuffs.setBase : baseSetDmg)}</td><td class="mt-cell-val"></td></tr>` : ''}
                    ${(data.detailedBuffs?.accessoryBase || 0) > 0 ? `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Accessory Base <span style="font-size: 0.65rem; color:#a78bfa;">(${cleanHeadDisplayName})</span></td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(data.detailedBuffs.accessoryBase)}</td><td class="mt-cell-val"></td></tr>` : ''}
                    ${(() => {
            ...
            return `<tr><td class="mt-cell-label mt-pl-md opacity-70">${label} <span style="font-size:0.65rem; color:#a78bfa;">(${cleanHeadDisplayName})</span></td><td class="mt-cell-formula" style="vertical-align: middle; white-space:nowrap;">${fmt.pct(val)}</td><td class="mt-cell-val"></td></tr>`;
        })()}
                    ${(() => {
            const val = (data.detailedBuffs ? data.detailedBuffs.tagBonus : tagDmg);
            if (val === 0) return '';
            return `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Tag Bonus</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(val)}</td><td class="mt-cell-val"></td></tr>`;
        })()}
                    ${(() => {
            ...
            if (data.detailedBuffs.setPerk && data.detailedBuffs.setPerk !== 0) {
                ...
                if (data.relicStats && data.relicStats.set === 'monarch') {
                    ...
                    label = `↳ Set Perks (Monarch: +10% × ${summons} Summons)`;
                } else if (data.relicStats && data.relicStats.set === 'warlord') {
                    label = `↳ Set Perks (Warlord)`;
                }
                html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">${label}</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(data.detailedBuffs.setPerk)}</td><td class="mt-cell-val"></td></tr>`;
            }
            return html;
        })()}
                    ${(() => {
            ...
            if (data.detailedBuffs.passiveBreakdown && data.detailedBuffs.passiveBreakdown.length > 0) {
                data.detailedBuffs.passiveBreakdown.forEach(p => {
                    if (p.dmg !== 0) html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ ${p.name}</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(p.dmg)}</td><td class="mt-cell-val"></td></tr>`;
                    html += getAbhContributionRowsHtml(p, data, 'table');
                });
                const namedDmg = data.detailedBuffs.passiveBreakdown.reduce((sum, p) => sum + (p.dmg || 0), 0);
                const eternalSub = (data.traitObj && data.traitObj.isEternal) ? (Math.min(data.wave || 12, 12) * 5) : 0;
                const rem = (data.detailedBuffs.unitPassive || 0) - namedDmg - eternalSub;
                if (Math.abs(rem) > 0.01) html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Unit Base</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(rem)}</td><td class="mt-cell-val"></td></tr>`;
            } else {
                const eternalSub = (data.traitObj && data.traitObj.isEternal) ? (Math.min(data.wave || 12, 12) * 5) : 0;
                const val = (data.detailedBuffs ? data.detailedBuffs.unitPassive : passiveDmg) - eternalSub;
                if (Math.abs(val) > 0.01) html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Unit Passive</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(val)}</td><td class="mt-cell-val"></td></tr>`;
            }
            return html;
        })()}
                    ${eternalDmg > 0 ? `<tr><td class="mt-cell-label mt-pl-md text-accent-start opacity-70">↳ Eternal (Wave 12+)</td><td class="mt-cell-formula text-accent-start">${fmt.pct(eternalDmg)}</td><td class="mt-cell-val"></td></tr>` : ''}
                    ${(data.abilityBuff || 0) > 0 ? `<tr><td class="mt-cell-label mt-pl-md text-custom opacity-70">↳ Ability Buffs</td><td class="mt-cell-formula text-custom" style="white-space:nowrap;">${fmt.pct(data.abilityBuff)}</td><td class="mt-cell-val"></td></tr>` : ''}
                    ${globalDmgBreakdownHtml}
                    ${data.conditionalData ? `<tr><td class="mt-cell-label mt-pt-md mt-text-orange mt-text-bold">${data.conditionalData.name}</td><td class="mt-cell-formula mt-pt-md mt-text-orange mt-text-bold">x${data.conditionalData.mult.toFixed(2)}</td><td class="mt-cell-val calc-highlight mt-pt-md">${fmt.num(data.dmgVal)}</td></tr>` : ''}
                </table>
            </div>`;
}
...
function getTagPerkRowsHtml(statType, data) {
    ...
    checkSet.forEach(item => {
        let setId = item.id;
        if (setId.includes('shadow_reaper')) setId = 'shadow_reaper';
        else if (setId.includes('reaper')) setId = 'reaper_set';
        else if (setId.includes('fused_earring')) setId = 'fused_earrings_acc';
        else if (setId === 'fused' || setId === 'fused_set') setId = 'fused_set';
        ...
        if (window.TAG_PERKS?.[setId]) {
            window.TAG_PERKS[setId].forEach(perk => {
                if (unitTags.includes(perk.tag)) {
                    ...
                    if (val) {
                        html += `<tr><td class="mt-cell-label mt-pl-lg text-dim text-xs" style="line-height:1.3;">↳ Tag Bonus<br>&nbsp;&nbsp;&nbsp;&nbsp;(${perk.tag} : ${setId.replace('_', ' ').toUpperCase()})</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-dim text-xs" style="vertical-align:top; padding-top:2px;">+${fmt.fix(val, 1)}%</td></tr>`;
                    }
                }
            });
        }
            ...
            perks.forEach(perk => {
                if (unitTags.includes(perk.tag)) {
                    ...
                    if (dotVal) {
                        ...
                        html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Tag Bonus: ${perk.tag} (${cleanSet})</td><td class="mt-cell-formula">${fmt.pct(dotVal)}</td><td class="mt-cell-val"></td></tr>`;
                    }
                }
            });
        }
...
function renderCritSection(data, setTagCfTotal, setTagCmTotal, cleanHeadDisplayName, relicSetName) {
    ...
    if (data.activeGlobalBuffs && window.GLOBAL_BUFF_DATA) {
        Object.values(window.GLOBAL_BUFF_DATA).forEach(buff => {
            const bData = data.activeGlobalBuffs[buff.id];
            if (bData) {
                if (bData.crit > 0) globalCritBreakdownHtml += `<tr><td class="mt-cell-label mt-pl-lg text-dim text-xs" style="color:${buff.color}">• ${buff.name}</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-dim text-xs" style="color:${buff.color}; white-space:nowrap;">+${fmt.fix(bData.crit / (data.bpMult || 1), 1)}%</td></tr>`;
                if (bData.cdmg > 0) globalCdmgBreakdownHtml += `<tr><td class="mt-cell-label mt-pl-lg text-dim text-xs" style="color:${buff.color}">• ${buff.name}</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-dim text-xs" style="color:${buff.color}; white-space:nowrap;">+${fmt.fix(bData.cdmg / (data.bpMult || 1), 1)}%</td></tr>`;
            }
        });
    }

    let passiveCritBreakdownHtml = '', passiveCdmgBreakdownHtml = '';
    ...
    return `
            <div class="dd-section">
                ...
                <table class="calc-table">
                    ...
                    ${globalCritBreakdownHtml}${passiveCritBreakdownHtml}
                    ...
                    <tr><td class="mt-cell-label mt-pl-sm mt-text-gold mt-text-bold mt-pt-md">Crit Damage</td><td class="mt-cell-formula mt-pt-md"></td><td class="mt-cell-val mt-pt-md"></td></tr>
                    ...
                    ${relicCdmgVal > 0 ? `<tr><td class="mt-cell-label mt-pl-lg text-dim text-xs">• Relics (Main + Sub)</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-dim text-xs">+${fmt.fix(relicCdmgVal, 1)}%</td></tr>` : ''}
                    ${setCmVal > 0 ? `<tr><td class="mt-cell-label mt-pl-lg text-dim text-xs" style="line-height:1.4;">↳ Set Bonus<br>&nbsp;&nbsp;&nbsp;&nbsp;(${relicSetName})</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-dim text-xs" style="vertical-align:top; padding-top:4px;">+${fmt.fix(setCmVal, 1)}%</td></tr>` : ''}
...
function renderSpaSection(data, traitRowsSpa, baseSetSpa, tagSpa, passiveSpa, cleanHeadDisplayName) {
    ...
    if (data.activeGlobalBuffs && window.GLOBAL_BUFF_DATA) {
        Object.values(window.GLOBAL_BUFF_DATA).forEach(buff => {
            ...
            if (bData && bData.spa > 0) {
                globalSpaBreakdownHtml += `<tr><td class="mt-cell-label mt-pl-md opacity-70" style="color:${buff.color};">↳ ${buff.name}</td><td class="mt-cell-formula" style="color:${buff.color}; white-space:nowrap;">${fmt.pct(-(bData.spa / (data.bpMult || 1)))}</td><td class="mt-cell-val"></td></tr>`;
            }
        });
    }
    ...
    return `
            <div class="dd-section">
                <div class="dd-title mt-text-custom"><span>3. SPA Calculation</span> <button class="calc-info-btn" onclick="openInfoPopup('spa_calc')">?</button></div>
                <table class="calc-table">
            ...
            if (data.detailedBuffs && data.detailedBuffs.passiveBreakdown && data.detailedBuffs.passiveBreakdown.length > 0) {
                data.detailedBuffs.passiveBreakdown.forEach(p => {
                    if (p.spa !== 0) html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ ${p.name}</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(-p.spa)}</td><td class="mt-cell-val"></td></tr>`;
                });
                const namedSpa = data.detailedBuffs.passiveBreakdown.reduce((sum, p) => sum + (p.spa || 0), 0);
                const rem = passiveSpa - namedSpa;
                if (Math.abs(rem) > 0.01) html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Unit Passive</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(-rem)}</td><td class="mt-cell-val"></td></tr>`;
            } else {
                if (passiveSpa !== 0) html = `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Unit Passive</td><td class="mt-cell-formula" style="white-space:nowrap;">${fmt.pct(-passiveSpa)}</td><td class="mt-cell-val"></td></tr>`;
            }
            return html;
        })()}
...
function renderDotSection(data, headDotRow) {
    if (data.dotData && data.dotData.inactive) {
        ...
        <div class="dd-section">
            ...
            <div style="background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.3); padding:12px; border-radius:8px; color:#fca5a5; font-size:0.75rem; text-align:center; margin-top:8px; line-height:1.4;">
                ...
                This unit requires a teammate with <b style="color:#fff; text-decoration:underline;">${data.dotData.requirement}</b> DoT to enable its own status effects.
            </div>
        </div>`;
    }
    ...
    const baseDot = headDotOverride || data.baseStats?.dot || (db?.base || 0) || 0;
    ...
    // externalDotBonus = set bonus + accessory (set/tag/head only)
    const externalDotBonus = db?.externalDotBonus || 0;
    const globalDotBonus = db?.globalDotBonus || 0;
    const bugMult = (data.baseStats?.id === 'ant_king_savage' || (window.isUnit && window.isUnit(data.baseStats?.id, 'ant_king_savage'))) ? 2 : 1;
    const traitMult = 1 + (traitDot / 100);
    const relicMult = 1 + (relicDotBonus / 100);
    const externalMult = 1 + ((externalDotBonus * bugMult) / 100);
    const globalMult = 1 + (globalDotBonus / 100);
    const passiveMult = 1 + (passiveDot / 100);
    const combinedMultiplier = traitMult * relicMult * externalMult * globalMult * passiveMult * (db?.globalDotMult || 1.0);
    ...
    if (data.headBuffs && data.headBuffs.type === 'ninja') {
        ...
        <tr class="mt-row-ninja"><td colspan="3" class="p-2">
            <div class="mt-flex-between mb-2"><span class="text-custom mt-text-bold text-xs tracking-sm">NINJA HEAD PASSIVE</span><button class="calc-info-btn" onclick="openInfoPopup('ninja_passive')">?</button></div>
            <div class="mt-flex-between text-xs text-white mb-1"><span class="opacity-70">Active Duration:</span><span class="mt-font-mono text-white">10.0s</span></div>
            <div class="mt-flex-between text-xs text-white mb-3"><span class="opacity-70">Uptime:</span><span class="mt-font-mono mt-text-green">${fmt.fix(uptimePct * 100, 1)}%</span></div>
            <div class="mt-flex-between mt-border-top mt-pt-sm"><span class="text-white text-xs text-bold">Avg DoT Buff</span><span class="text-custom text-sm mt-text-bold"> +${fmt.fix(data.headBuffs.dot, 2)}%</span></div>
        </td></tr>`;
    }
    ...
    if (data.headBuffs && data.headBuffs.type === 'reanimated') {
        ...
        <tr class="mt-row-sungod"><td colspan="3" class="p-2">
            <div class="mt-flex-between mb-2"><span class="text-accent-end mt-text-bold text-xs tracking-sm">REANIMATED PASSIVE</span><button class="calc-info-btn" onclick="openInfoPopup('reanimated_passive')">?</button></div>
            <div class="mt-flex-between text-xs text-white mb-1"><span class="opacity-70">Range Stat:</span><span class="mt-font-mono mt-text-range">${fmt.fix(data.range, 1)}</span></div>
            <div class="mt-flex-between text-xs text-white mb-1"><span class="opacity-70">Trigger:</span><span class="mt-font-mono text-white">5th Attack (${fmt.fix(data.headBuffs.trigger, 1)}s)</span></div>
            <div class="mt-flex-between text-xs text-white mb-3"><span class="opacity-70">Uptime:</span><span class="mt-font-mono mt-text-green">${fmt.fix(uptimePct * 100, 1)}%</span></div>
            <div class="mt-flex-between mt-border-top mt-pt-sm"><span class="text-white text-xs text-bold">Avg DoT Buff</span><span class="text-accent-end text-sm mt-text-bold"> +${fmt.fix(data.headBuffs.dot, 2)}%</span></div>
        </td></tr>`;
    }
    ...
    if (data.headBuffs && (data.headBuffs.type === 'almighty_accessory' || data.headBuffs.type === 'almighty')) {
        const bpTotal = data.headBuffs.buffPotency || 0;
        const activeTags = data.headBuffs.activeTags || [];
        const tagRows = activeTags.map(tag => {
            const perkList = (typeof TAG_PERKS !== 'undefined') ? (TAG_PERKS.almighty_acc || []) : [];
            const perk = perkList.find(p => p.tag === tag);
    ...
    <div class="dd-section">
        ...
        <table class="calc-table">
            ...
            ${headDotRow}
            ${db?.nativeDps > 0 ? `
            <tr><td class="mt-cell-label mt-pt-md">Native Tick %</td><td class="mt-cell-formula mt-pt-md"></td><td class="mt-cell-val mt-pt-md mt-text-bold">${fmt.fix(baseDotPctVal, 1)}%</td></tr>
            ${baseDot > 0 ? `<tr><td class="mt-cell-label mt-pl-sm opacity-70">↳ ${headDotOverride > 0 ? `Head Piece Override (${headOverrideLabel})` : ((window.isUnit && window.isUnit(data.baseStats?.id, 'the_almighty')) ? 'Reishi Manipulation' : 'Unit Base')}</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-xs text-white">${fmt.num((window.isUnit && window.isUnit(data.baseStats?.id, 'the_almighty')) ? baseDotPctVal : baseDot)}%</td></tr>` : ''}
            ${(() => {
                if (data.detailedBuffs && data.detailedBuffs.passiveBreakdown) {
                    // For the_almighty, per-unit dot scaling is baked into the base dot row — skip additive display
                    const isAlmighty = window.isUnit && window.isUnit(data.baseStats?.id, 'the_almighty');
                    let html = '';
                    const seen = new Set();
                    data.detailedBuffs.passiveBreakdown.forEach(p => {
                        if (p.dot > 0 && !seen.has(p.name) && !isAlmighty) {
            ...
            <tr><td class="mt-cell-label mt-pt-md">Base DoT Damage</td><td class="mt-cell-formula mt-pt-md">=</td><td class="mt-cell-val mt-pt-md text-gray mt-text-bold">${fmt.num((data.dmgVal * (db?.critMult || 1)) * (baseDotPctVal / 100))}</td></tr>
            <tr><td class="mt-cell-label mt-pl-sm mt-text-bold text-custom">1. Trait Multiplier</td><td class="mt-cell-formula"><span class="op">×</span>${fmt.fix(traitMult, 3)}</td><td class="mt-cell-val text-custom text-bold">${fmt.pct(traitDot)}</td></tr>
            ${relicDotBonus > 0 ? `<tr><td class="mt-cell-label mt-pl-sm mt-text-bold text-accent-end">2. Relic Main/Sub Multiplier</td><td class="mt-cell-formula"><span class="op">×</span>${fmt.fix(relicMult, 3)}</td><td class="mt-cell-val text-accent-end text-bold">${fmt.pct(relicDotBonus)}</td></tr>` : ''}
            ...
            ${(() => {
                ...
                if (externalDotBonus > 0) {
                    ...
                    if (data.detailedBuffs?.dotSetBase > 0) {
                        html += `<tr><td class="mt-cell-label mt-pl-md opacity-70">↳ Set Base DoT</td><td class="mt-cell-formula">${fmt.pct(data.detailedBuffs.dotSetBase)}</td><td class="mt-cell-val"></td></tr>`;
                    }
                    html += getTagPerkRowsHtmlForDot(data);
                    ...
                    if (data.activeGlobalBuffs && window.GLOBAL_BUFF_DATA) {
                        Object.values(window.GLOBAL_BUFF_DATA).forEach(buff => {
                            ...
                            if (bData && bData.dot > 0) {
                                html += `<tr><td class="mt-cell-label mt-pl-md opacity-70" style="color:${buff.color};">↳ ${buff.name}</td><td class="mt-cell-formula" style="color:${buff.color};">${fmt.pct(bData.dot)}</td><td class="mt-cell-val"></td></tr>`;
                            }
                        });
                    }
                    nextIdx++;
            ...
            <tr class="mt-border-top"><td class="mt-cell-label mt-pl-sm mt-pt-sm text-bold text-gray">↳ Combined DoT Multiplier</td><td class="mt-cell-formula mt-pt-sm text-bold"><span class="op">×</span>${fmt.fix(db?.combinedMultiplier || combinedMultiplier, 3)}</td><td class="mt-cell-val"></td></tr>
            ...
            ${(() => {
                let nextIdx = (db.relicBonus || 0) > 0 ? 3 : 2;
                let html = '';
            ...
            ${data.placement > 1 ? `
            <tr>
                <td class="mt-cell-label text-gold">Total x${data.placement} Units</td>
...
function renderAttackRateSection(data) {
            ...
            if (f.nextAttack && ea) {
                ...
                return `
                    <div class="dd-section" style="border-left:3px solid #4ade80;">
                        <div class="dd-title mt-text-green"><span>5. Attack Cycle Snapping</span> <button class="calc-info-btn" onclick="openInfoPopup('attack_rate')">?</button></div>
                        <table class="calc-table">
    ...
    if (isFW && data.extraAttacks) {
        ...
        return `
            <div class="dd-section" style="border-left:3px solid #4ade80;">
                <div class="dd-title mt-text-green"><span>5. Fused Godly Might</span> <button class="calc-info-btn" onclick="openInfoPopup('attack_rate')">?</button></div>
                <table class="calc-table">
                    ...
                    <tr><td class="mt-cell-label text-custom">1st Follow-Up Attack</td><td class="mt-cell-formula">100% Proc Rate</td><td class="mt-cell-val text-custom">+1.0 Hits</td></tr>
                    <tr><td class="mt-cell-label text-accent-start">2nd Follow-Up Attack</td><td class="mt-cell-formula">${fua2Chance}% Proc</td><td class="mt-cell-val text-accent-start">+${fua2Chance / 100} Hits</td></tr>
                    <tr class="mt-border-top">
                        <td class="mt-cell-label text-white">Average Hits</td>
    ...
    if (data.baseStats?.id === 'triple_threat') {
        ...
        return `
            <div class="dd-section" style="border-left:3px solid #60a5fa;">
                <div class="dd-title text-custom" style="color:#60a5fa !important;"><span>5. Brutal Slashes Follow-Up</span> <button class="calc-info-btn" onclick="openInfoPopup('tag_logic')">?</button></div>
                <table class="calc-table">
                    <tr><td class="mt-cell-label">Trigger Condition</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-white">Critical Bleed enemy hit</td></tr>
                    <tr><td class="mt-cell-label">Trigger Cooldown</td><td class="mt-cell-formula"></td><td class="mt-cell-val text-white">Every 15s</td></tr>
                    <tr><td class="mt-cell-label">FUA DPS (x${data.placement})</td><td class="mt-cell-formula">${fmt.num(singleFuaDps)} × ${data.placement}</td><td class="mt-cell-val text-accent-start font-bold" style="font-size:1.05rem;">${fmt.num(totalFuaDps)}</td></tr>
                </table>
            </div>`;
    }
    ...
    if (data.baseStats?.id === 'strongest_swordsman_hunter' && data.extraAttacks) {
        ...
        return `
            <div class="dd-section" style="border-left:3px solid #4ade80;">
                ...
                <table class="calc-table">
                    <tr><td class="mt-cell-label">Stance 1 & 3 (6 Normal)</td><td class="mt-cell-formula">Crit: ${fmt.fix(ea.unbuffedCrit, 1)}%</td><td class="mt-cell-val" style="color:#94a3b8;">${fmt.num(ea.unbuffedHitVal)} Avg</td></tr>
                    <tr><td class="mt-cell-label text-custom" style="font-weight:700;">Stance 2 (3 Buffed)</td><td class="mt-cell-formula text-custom" style="font-weight:700;">+60% Dmg / +20% Crit</td><td class="mt-cell-val text-custom" style="font-weight:700;">${fmt.num(ea.buffedHitVal)} Avg</td></tr>
                    <tr class="mt-border-top">
                        <td class="mt-cell-label text-white mt-pt-sm">Final Stance DPS Mult</td>
...
function renderSummonSection(data) {
    ...
    return `
    <div class="dd-section">
        <div class="dd-title text-accent-start"><span>${isNutaru ? 'Clones' : 'Summon Logic'}</span></div>
        <table class="calc-table">
            <tr><td class="mt-cell-label">${isNutaru ? 'Single Clone Dmg' : 'Plane Damage'}</td><td class="mt-cell-val">${isNutaru ? fmt.num((data.dmgVal || 0) * 0.75) : fmt.num((data.dmgVal || 0) * (data.baseStats?.summonStats?.dmgPct / 100 || 0.5))}</td></tr>
            <tr><td class="mt-cell-label">${isNutaru ? 'Summon Rate' : 'Spawn Rate'}</td><td class="mt-cell-val">${fmt.fix((data.summonData?.hostSpa || 0) * (isNutaru ? 8 : 1), 2)}s</td></tr>
            ...
            <tr><td class="mt-cell-label mt-pt-md">Avg DPS (Individual)</td><td class="mt-cell-val mt-pt-md">${fmt.num(data.summonData?.avgPlaneDps)}</td></tr>
...
function renderFinalSection(data) {
    ...
    if (data.baseStats?.id === 'triple_threat') {
        ...
        tripleThreatFollowUpHtml = `
            ...
            <tr><td class="mt-cell-label mt-pl-md text-accent-start" style="font-weight: 700;">↳ Brutal Slashes FUA</td><td class="mt-cell-formula text-accent-start">${fmt.num(singleFuaDps)} <span class="op">×</span> ${data.placement}</td><td class="mt-cell-val text-accent-start" style="font-weight:700;">${fmt.num(totalFuaDps)}</td></tr>
        `;
    }
    ...
    return `
            <div class="dd-section border-l-gold">
                ...
                <table class="calc-table">
                    ...
                    ${data.summon > 0 ? `<tr><td class="mt-cell-label">Summon DPS</td><td class="mt-cell-formula">+</td><td class="mt-cell-val text-accent-start">${fmt.num(data.summon)}</td></tr>` : ''}
                    <tr class="mt-border-top">
...
window.renderMathContent = function (data, isSplit = false) {
        ...
        return {
            content: `
                <div class="breakdown-center-panel" style="display:flex; flex-direction:column; gap:15px;">
                    ${baseDamage}${critSection}${spaSection}${rangeSection}${dotSection}${attackRate}${finalSection}
                </div>
            `,
            leftPanel: `
                <div class="breakdown-left-panel" style="display:flex; flex-direction:column; gap:15px;">
                    ${overview}${summary}${quickBreakdown}${activeBuffs}
                </div>
            `,
            rightPanel: `
    ...
    return `
        <div class="math-breakdown-single-column" style="display:flex; flex-direction:column; gap:15px;">
            ${overview}${summary}${quickBreakdown}${activeBuffs}${baseDamage}${critSection}${spaSection}${rangeSection}${dotSection}${attackRate}${summonSection}${finalSection}
        </div>
    `;
};
