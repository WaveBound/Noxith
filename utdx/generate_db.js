...
if (isMainThread) {
    ...
    function fastCalculateUnitBuilds(unit, cfg, traitsForCalc, isAbility, existingHeads) {
        ...
        traitsForCalc.forEach(trait => {
            ...
            const pushBest = (map, prio) => {
                for (const best of map.values()) {
                    traitGroups[trait.name].push({
                        ...
                        traitName: trait.name, dps: best.res.total, bossDps: best.res.bossTotal, dmgVal: best.res.dmgVal,
            ...
            const getScore = (r) => Math.max(r.total || 0, r.bossTotal || 0);
            ...
            try {
                ...
                for (let i = 0; i < tmplLen; i++) {
                    ...
                    const cdScore = cd ? getScore(cd.res) : 0;
                    
                    if (!cd || score > cdScore) bestDps.set(key, { res, meta: t.meta });
                    
                    const cr = bestRaw.get(key);
                    if (!cr || res.dmgVal > cr.res.dmgVal || (res.dmgVal === cr.res.dmgVal && score > getScore(cr.res))) bestRaw.set(key, { res, meta: t.meta });
                }
            ...
            try {
                ...
                for (let i = 0; i < tmplLen; i++) {
                    ...
                    const cScore = c ? getScore(c.res) : 0;
                    
                    if (!c || score > cScore) bestSpa.set(key, { res, meta: t.meta });
                }
            ...
            try {
                ...
                if (allowRange) {
                    ...
                    for (let i = 0; i < tmplLen; i++) {
                        ...
                        if (!c || res.range > c.res.range || (res.range === c.res.range && score > cScore)) bestRange.set(key, { res, meta: t.meta });
                    }
