...
if (isMainThread) {
    ...
    function getEffectiveCritCapForGenerator(effectiveStats, critRes, headPiece = 'none') {
        ...
        if (isUnit(effectiveStats.id, 'angel_born_in_hell')) return 0;
        if (isUnit(effectiveStats.id, 'the_strongest_of_today') || isUnit(effectiveStats.id, 'strongest_of_today')) return 50;
    ...
    function fastCalculateUnitBuilds(unit, cfg, traitsForCalc, isAbility, existingHeads, jobHeads, selectedSets, includeRangeSubs = false) {
        ...
        let allowedHeads = cfg.head ? (jobHeads && jobHeads.length > 0 ? jobHeads : ['sun_god', 'ninja', 'reaper_necklace', 'shadow_reaper_necklace', 'junior', 'biju_head', 'bloodline_head', 'reanimated_head', 'sorcerer_hunter_spirit', 'strongest_sorcerer_glasses', 'monarch', 'warlord_hat', 'mochi_scarf', 'flaming_donut', 'ultiorras_wings', 'berserks_cleave', 'panther_claws', 'fused_earrings', 'koyotes_sword']) : ['none'];
    ...
    function finalizeDatabase(rawDb, existingRaw, outPath, sigHeads) {
        ...
        const MAP_HEAD = { 'none': 0, 'sun_god': 1, 'ninja': 2, 'reaper_necklace': 3, 'shadow_reaper_necklace': 4, 'junior': 5, 'biju_head': 6, 'bloodline_head': 7, 'reanimated_head': 8, 'sorcerer_hunter_spirit': 9, 'strongest_sorcerer_glasses': 10, 'monarch': 11, 'warlord_hat': 12, 'mochi_scarf': 13, 'flaming_donut': 14, 'ultiorras_wings': 15, 'berserks_cleave': 16, 'panther_claws': 17, 'fused_earrings': 18, 'koyotes_sword': 19, 'phantom_stealer_head': 20 };
...
""")
            ...
            def existing_file_has_latest_heads(fpath):
            ...
            if mode == 'missing':
                ...
                for c in all_combos:
                    ...
                    if selected_units:
                        if not existing_file_has_units(fpath, selected_units) or not existing_file_has_latest_heads(fpath):
            ...
            if selected_heads is None:
                selected_heads = ['sun_god', 'ninja', 'reaper_necklace', 'shadow_reaper_necklace', 'junior', 'biju_head', 'bloodline_head', 'reanimated_head', 'sorcerer_hunter_spirit', 'strongest_sorcerer_glasses', 'monarch', 'warlord_hat', 'mochi_scarf', 'flaming_donut', 'ultiorras_wings', 'berserks_cleave', 'panther_claws', 'fused_earrings', 'koyotes_sword']
                ...
                for c in all_combos:
                    ...
                    if selected_units:
                        ...
                        try:
                            with open(fpath, "r", encoding="utf-8") as f:
                                first_lines = "".join([f.readline() for _ in range(5)])
                                required_head_sigs = ["flaming_donut", "fused_earrings", "ultiorras_wings", "berserks_cleave", "panther_claws"]
                                if "// BUILDSIG:" not in first_lines or any(h not in first_lines for h in required_head_sigs):
                                    combinations.append(c)
                        except Exception:
                            combinations.append(c)
...
<body>
    ...
    <script>
        ...
        const relicSets = ['Junior Ninja', 'Sun God', 'Laughing Captain', 'Ex Captain', 'Shadow Reaper', 'Reaper Set', 'Super Roku', 'Bio-Android', 'Biju Set', 'Rebellious Set', 'Reanimated Set', 'Great Mage', 'Sorcerer Hunter', 'Strongest Sorcerer', 'Monarch', 'Warlord', 'Fused Warrior', 'Phantom Stealer'];
