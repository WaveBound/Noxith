...
function renderSourceTotalsSection(data) {
    ...
    const waterGodDmg = data.waterGodBuff || 0;
    const bijuuDmg = data.bijuuBuff || 0;
    const kingMarkDmg = data.kingMarkDmg || 0;
    const passiveTotalDmg = unitInnateDmg + abilityDmg + accessoryDmg + mikuDmg + waterGodDmg + bijuuDmg + kingMarkDmg;

    const passiveTotalSpa = (data.passiveSpaBuff || 0) + (data.waterGodSpa || 0) + (data.bijuuSpa || 0) + (data.kingMarkSpa || 0) + (data.mageHillSpa || 0);
    const passiveTotalRange = (data.baseStats.passiveRange || 0) + (data.eternalRangeBuff || 0) + (data.waterGodBuff || 0) + (data.bijuuBuff || 0);
...
function renderRangeSection(data) {
    ...
    const totalAdditiveRange = (data.totalSetStats.range || 0) + (data.passiveRange || 0) + (data.waterGodBuff || 0);
